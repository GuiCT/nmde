...
# ╔═╡ 0fa94fa8-041d-11f0-1bba-1dc5ec043b34
using DataFrames, Plots, XLSX

# ╔═╡ 26ee7ece-5bcd-4b07-903e-b52e0ee94e81
using PlutoUI

# ╔═╡ 65ddce95-e9c3-40eb-82f9-9f30fea9bdf3
# Diferença avançada
forward_diff(y, x0, h) = (y(x0 + h) - y(x0)) / h

# ╔═╡ 2b18cdf1-11ca-4d8b-a340-4d291dc37213
# Diferença atrasada
backwards_diff(y, x0, h) = (y(x0) - y(x0 - h)) / h

# ╔═╡ 9dbecbaa-7e1e-4682-9269-0978f881e4d8
# Diferença centrada
centered_diff(y, x0, h) = (y(x0 + h) - y(x0 - h)) / (2 * h)

# ╔═╡ 96263555-33b7-453b-bd73-2ab393bfe03d
@bind func TextField()

# ╔═╡ 04b34c16-cddf-4429-a7c1-d615a9aa11db
main_func = eval(Meta.parse(func))

# ╔═╡ 5d315c70-a46c-41a0-9bd3-51432ab74b01
@bind deriv_analytic TextField()

# ╔═╡ cf147076-294b-42dd-ab2e-825ef00d9832
deriv_func = eval(Meta.parse(deriv_analytic))

# ╔═╡ ac9459da-8721-432c-9d82-2c4d8300ac9a
h_list = LinRange(
    (10.0 ^ (-1)),
    (10.0 ^ (-8)),
    100
...
# ╔═╡ 88e335a7-ee88-4a51-bd45-471298bca7eb
@bind X₀ Slider(0:0.01:10)

# ╔═╡ f60e76ce-a277-4ca0-8389-e91eaaf2b16a
X₀

# ╔═╡ 4f2415cd-4351-473b-895e-067329734500
expected_value = deriv_func(X₀)

# ╔═╡ 06c867e9-5369-4ba9-a370-fa155d828063
begin
	df = DataFrame(
	    Tipo=String[],
	    Passo=Float64[],
	...
	for h in h_list
	    val_forwards = forward_diff(main_func, X₀, h)
	    val_backwards = backwards_diff(main_func, X₀, h)
	    val_centered = centered_diff(main_func, X₀, h)
...
manifest_format = "2.0"
project_hash = "39fa0634d8fa9a7bce69c1fa771cc7337d0ae6ac"
