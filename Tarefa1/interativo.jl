...
# ╔═╡ b746f6ac-08dc-11f0-1283-6da72060bee5
begin
	using DataFrames
	using Printf
	using Plots
...
# ╔═╡ 6cf9112e-9775-4aa1-8089-dbac5201d5f0
begin
	# Diferença avançada
...
# ╔═╡ 61492053-9adc-4601-b233-34e5a0fc7a5e
@syms x

# ╔═╡ 74862eb1-5673-46c2-9652-930c74be9a46
Dx = Differential(x)

# ╔═╡ 386d3c97-c8ea-4de2-88f4-28fa846df1cf
region_start = @bind r1 Slider(-100:0.01:100; show_value=true)

# ╔═╡ 5c55ecf7-ef82-4a28-bcb9-f373e158ce16
region_end = @bind r2 Slider(-100:0.01:100; show_value=true)

# ╔═╡ 5cb6cc05-8faf-427b-a0cd-d28614a0fcc1
dist_entre_pontos = 0.1

# ╔═╡ 5ff57c24-b896-4ad5-bb41-d34f31043785
slider_h = @bind h Slider(0.00001:0.00001:0.1; show_value=true)

# ╔═╡ 27f849e4-0127-401b-a25f-802c9c9dddd6
tf_func = @bind func_str TextField()

# ╔═╡ ce16d878-bb14-40f8-b240-a00044914bc9
func = func_str |> Meta.parse |> eval

# ╔═╡ 4411acc0-be50-48a7-9391-d97c107c877b
derivative = expand_derivatives(Dx(func))

# ╔═╡ 5003d617-160f-47e4-a9c8-38a485127025
func_callable = build_function(func, [x]) |> eval

# ╔═╡ 53c10aca-9f91-491e-9246-9ef307322473
derivative_callable = build_function(derivative, [x]) |> eval

# ╔═╡ 1f43b247-5324-41b1-be45-1abb21549b1e
begin
	df = DataFrame(
		x=Float64[],
		ValorReal=Float64[],
	...
	points_x = r1:dist_entre_pontos:r2
	for x_p in points_x
		expected_value = derivative_callable(x_p)
		fw = forward_diff(func_callable, x_p, h)
		bw = backwards_diff(func_callable, x_p, h)
		c_val = centered_diff(func_callable, x_p, h)
		push!(df,
			[x_p expected_value fw bw c_val]
		)
	end
end

# ╔═╡ 19d3a18b-361c-4e9e-9d4c-cff2efdf19c8
p = plot(
	df.x,
	[
		df.ValorReal,
	...
	title = "Estimativa de \$y'(x)\$ no intervalo [$(r1), $(r2)]\n\$y = $(func),h=$(h)\$,  \$y'(x)=$(derivative)\$",
	xlabel = "x",
	ylabel = "Valor da função",
	dpi = 600,
);

# ╔═╡ 51c5e0bf-7df9-4d75-9ddd-b8a167e47a0e
...
manifest_format = "2.0"
project_hash = "dd92453d00ee73b7320fc31c7a203504767664b6"
...
git-tree-sha1 = "e28016d1df9f71d0563dcc0cdbb46ee961479e2c"
...
git-tree-sha1 = "0287220df13e43aa502704ddb508a4fa579651e9"
